...
fzf_conventional_commit-widget() {
    commit_type=("${(f)"$(grep -vE '^#' $ZDOTDIR/external/fzf-conventional-commit/commit-type)"}")
    commit_emoji=("${(f)"$(grep -vE '^#' $ZDOTDIR/external/fzf-conventional-commit/commit-emoji)"}")
